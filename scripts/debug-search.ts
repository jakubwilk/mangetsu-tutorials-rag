...
type SearchResult = { id: string; content: string; documentTitle: string; category: string; rank: number };
...
const main = async () => {
  ...
  if (queryEmbedding) {
    ...
    embResults.forEach((r, i) => {
      console.log(`  [${i}] sim=${Number(r.rank).toFixed(4)} | ${r.documentTitle} | ${r.content.slice(0, 80).replace(/\n/g, "↵")}...`);
    });

    // 3. FTS with specific tokens (length > 2)
    const tokens = QUERY.toLowerCase().replace(/[^a-z0-9\sÀ-ſ]/g, " ").split(/\s+/).filter(t => t.length > 1);
    ...
    ftsResults.forEach((r, i) => {
      console.log(`  [${i}] rank=${Number(r.rank).toFixed(4)} | ${r.documentTitle} | ${r.content.slice(0, 80).replace(/\n/g, "↵")}...`);
    });
    ...
    merged.forEach(({ result: r, score }) => {
      console.log(`  score=${score.toFixed(5)} | ${r.documentTitle} | ${r.content.slice(0, 80).replace(/\n/g, "↵")}...`);
    });
  }
